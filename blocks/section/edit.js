			...
			<InspectorControls>
				<div className="ystdb-inspectorcontrols">
					...
					>
						...
						<BaseControl>
							<RangeControl
								label={ __( 'アニメーション開始までの時間', 'ystandard-blocks' ) }
						...
						<BaseControl>
							<Button
								...
								isPrimary
								onClick={ () => {
									setState( { previewAnimation: ! previewAnimation } );
									if ( ! previewAnimation ) {
										setTimeout( () => {
											setState( { previewAnimation: false } );
										}, ( animationDelay + animationSpeed ) * 1000 );
									}
								} }
							>
								{ __(
									previewAnimation ? 'アニメーションの停止' : 'アニメーションの確認',
									'ystandard-blocks'
								) }
							</Button>
						</BaseControl>
